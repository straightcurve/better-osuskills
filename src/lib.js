...
function getMaps(html, categories) {
    ...
    return anchors.map((i, elem) => {
        let mods = elem.next?.next?.childNodes[0]?.data || "NM";
        let name =
        ...
        return {
            ...
            memory,
            toString: function () {
                return `[${this.link} ${this.name}] ${this.mods} (${this.categories.join(", ")})`;
            },
        };
    }).toArray();
}
...
function bootstrapGetMaps(doc) {
     ...
     */
    const stamina = document.getElementById("staminaCheckbox");
    /**
     * @type {HTMLInputElement}
     */
    const tenacity = document.getElementById("tenacityCheckbox");
    /**
     * @type {HTMLInputElement}
     */
    const agility = document.getElementById("agilityCheckbox");
    /**
     * @type {HTMLInputElement}
     */
    const accuracy = document.getElementById("accuracyCheckbox");
    /**
     * @type {HTMLInputElement}
     */
    const precision = document.getElementById("precisionCheckbox");
    /**
     * @type {HTMLInputElement}
     */
    const reaction = document.getElementById("reactionCheckbox");
    /**
     * @type {HTMLInputElement}
     */
    const memory = document.getElementById("memoryCheckbox");
    ...
    ipc.on("log-maps", (e, message) => {
        console.log("Queued up", JSON.parse(message.serialized_maps));
    });
    ...
    function getCategories() {
        ...
        if (isStreamy())
            _categories.push("streamy");
        if (isTinyCircles())
            _categories.push("tiny circles");
        if (isRhythm())
            _categories.push("rhythm");
        if (isFastAim())
            _categories.push("fast aim");
        if (isSightReading())
            _categories.push("sight-reading");
        if (isMemory())
            _categories.push("memory");
    ...
    function isStreamy() {
        return (
            (!stamina.checked && !tenacity.checked)
        );
    }

    function isTinyCircles() {
        return (
            (!precision.checked)
        );
    }
